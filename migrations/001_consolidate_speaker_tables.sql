...
-- Date: 2025-01-11
-- Description: Consolidates two-table architecture into single table with proper 1:1 constraints
...
CREATE TABLE speakers_new (
    ...
    canonical_speaker_id INTEGER REFERENCES speakers_new(id),
...
-- Step 5: Update speaker_transcriptions foreign key references
-- First, create a mapping from old speaker IDs to new speaker IDs
...
-- Update speaker_transcriptions to reference the new speaker IDs
...
-- Step 6: Handle any SpeakerTranscription records that don't have corresponding embeddings
-- These will get placeholder speaker records
INSERT INTO speakers_new (
    content_id,
    local_speaker_id,
    global_id,
...
SELECT DISTINCT
    ...
    COALESCE(s.embedding, (SELECT embedding FROM speakers_new LIMIT 1)) as embedding,
    0.0 as embedding_quality_score,
    'legacy_migration' as algorithm_version,
    0.0 as duration,
    0 as segment_count,
    'PENDING' as rebase_status,
    COALESCE(s.meta_data, '{}'::jsonb) as meta_data,
...
-- Step 7: Update any remaining speaker_transcriptions references
UPDATE speaker_transcriptions st
...
-- Step 8: Drop old tables and rename new table
...
-- Step 9: Update sequence to match new table
-- Reset the sequence to continue from current max
SELECT setval('speaker_number_seq', 
    COALESCE((SELECT MAX(CAST(SUBSTRING(universal_name FROM 'Speaker (\d+)') AS INTEGER)) FROM speakers), 0) + 1);

-- Step 10: Re-create any triggers or functions that depend on the speakers table
-- (This would be customized based on your specific triggers)
